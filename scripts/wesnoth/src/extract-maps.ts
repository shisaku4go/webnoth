...
function parseMapGrid(
  filePath: string,
): { width: number; height: number; grid: string[][] } | null {
  try {
...
async function main() {
  ...
  for (const entry of campaignEntries) {
    ...
    try {
      ...
      const campaignName =
        cleanTranslation(getAttr(campaignNode, 'name')) ?? campaignId;
      ...
      if (existsSync(scenariosDir)) {
        const scenarioFiles = readdirSync(scenariosDir).filter((f) =>
          f.endsWith('.cfg'),
        );
        ...
        for (const sFile of scenarioFiles) {
          ...
          const scenarioNode =
            findChild(sTree, 'scenario') || findChild(sTree, 'test');
  ...
  if (existsSync(mpScenariosDir)) {
    const mpFiles = readdirSync(mpScenariosDir).filter((f) =>
      f.endsWith('.cfg'),
    );
    ...
    for (const f of mpFiles) {
      ...
      try {
        ...
        const mpNode =
          findChild(tree, 'multiplayer') || findChild(tree, 'scenario');
